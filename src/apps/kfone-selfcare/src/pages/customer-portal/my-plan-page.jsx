...
export const MyPlanPage = () => {
  ...
  useEffect(() => {
    ...
    getPackageRecommendation(decodedIDTokenPayload?.userid || decodedIDTokenPayload?.sub)
      .then(data => {
    ...
    return (
      ...
      >
        <div className="flex items-center">
          ...
          <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300">It&apos;s time to upgrade!</h3>
        </div>
        <div className="mt-2 mb-4 text-sm text-gray-700 dark:text-gray-300">
          We believe that you could benefit from a plan upgrade since you&apos;ve exceeded the existing plan thresholds
          continuously during past couple of months.
        </div>
        <div>
          <span className="font-small">We recommend the following {recommendation.name} package:</span>
  ...
  const renderPlanContent = () => (
    <>
      <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:gap-4 my-4">
        ...
        <div className="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
          <div id="usage-main-tab" className="tabs">
            ...
            >
              <span className={`text-xl font-bold${activeUsageTab === 1 ? 'text-black' : 'text-slate-400'}`}>
                Data Usage
              </span>
            </button>
            <button
            ...
            >
              <span className={`text-xl font-bold${activeUsageTab === 2 ? 'text-black' : 'text-slate-400'}`}>
                Talk Time Usage
              </span>
            </button>
          </div>
              ...
              {currentUsage ? (
                <>
                  <div id="data-tab" className="tabs tabs-boxed">
                    <button
                      className={`tab w-[50%] ${activeDataTab === 1 ? 'tab-active bg-teal-500' : ''}`}
                      onClick={() => handleDataTabNav(1)}
                    >
                      <span className={`text-sm ${activeDataTab === 1 ? 'text-light' : ''}`}>Data Bundle</span>
                    </button>
                    <button
                      className={`tab w-[50%] ${activeDataTab === 2 ? 'tab-active bg-teal-500' : ''}`}
                      onClick={() => handleDataTabNav(2)}
                    >
                      <span className={`text-sm ${activeDataTab === 2 ? 'text-light' : ''}`}>Additional Data</span>
                    </button>
                  </div>
                  <div className="flex flex-col items-center justify-evenly p-4 lg:p-8 space-y-4  h-full">
                    ...
                    <div className="text-center">
                      ...
                      GB remaining
                      <br />
                      <span className="text-sm font-light">as of {new Date().toLocaleString('en-US')}</span>
                    </div>
                  </div>
                </>
              ) : (
              ...
              {currentUsage ? (
                <>
                  <div id="talk-tab" className="tabs tabs-boxed">
                    <button
                      className={`tab w-[50%] ${activeTalkTab === 1 ? 'tab-active bg-sky-500' : ''}`}
                      onClick={() => handleTalkTabNav(1)}
                    >
                      <span className={`text-sm ${activeTalkTab === 1 ? 'text-light' : ''}`}>Talk Time</span>
                    </button>
                    <button
                      className={`tab w-[50%] ${activeTalkTab === 2 ? 'tab-active bg-sky-500' : ''}`}
                      onClick={() => handleTalkTabNav(2)}
                    >
                      <span className={`text-sm ${activeTalkTab === 2 ? 'text-light' : ''}`}>Additional Talk Time</span>
                    </button>
                  </div>
      ...
      <div className="w-full grid grid-cols-1 xl:grid-cols-2 xl:gap-4 my-4">
        <div className="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex-shrink-0">
              <a href="#" className="text-sm font-medium text-primary hover:underline rounded-lg p-2">
                View all
              </a>
            </div>
          </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white">
                      <tr>
                        ...
                        <td className="p-4 whitespace-nowrap text-sm font-semibold text-gray-900">$590</td>
                      </tr>
        ...
        <div className="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
          <h3 className="text-xl leading-none font-bold text-gray-900 mb-10">Buy Data Addons & Subscriptions</h3>
