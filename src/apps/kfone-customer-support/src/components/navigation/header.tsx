 ...
 */
export const Header: FC<HeaderProps> = ({className}: HeaderProps): ReactElement => {
  const classes = clsx('fixed bg-secondary-800 items-center h-20 w-full xl:w-[calc(100%-250px)] z-[999]', className);
      ...
      <div className="flex h-full px-3">
        <div className="justify-end space-x-4 items-center pl-2 flex w-full lg:max-w-68 sm:pr-2 sm:ml-0">
          ...
          <div className="relative">
            <button id="dropdownDefault" data-dropdown-toggle="dropdown" onClick={() => setIsOpen(!isOpen)}>
              <div className="avatar online placeholder">
                <div className="bg-primary text-gray-50 rounded-full w-16">
                  <span className="text-xl">
                    {state?.displayName?.split(' ') &&
                      `${state?.displayName?.split(' ')?.shift()?.charAt(0)}${state?.displayName
                        ?.split(' ')
                        ?.pop()
            ...
            {isOpen && (
              ...
              >
                <ul className="py-1 text-sm text-gray-700" aria-labelledby="dropdownDefault">
                  <li>
                    <span onClick={handleLogout} className="block py-2 px-4 hover:bg-gray-100 cursor-pointer">
                      Sign out
                    </span>
                  </li>
                </ul>
