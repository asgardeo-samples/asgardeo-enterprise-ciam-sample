...
import {PastBillingCycle, UserInfo} from '../models';
 ...
 */
export type CustomersPageProps = DetailedHTMLProps<HTMLAttributes<HTMLDivElement>, HTMLDivElement>;

/**
 * The Customers Page component.
 *
 * @returns The Customers Page component.
 */
export const CustomersPage: FC<CustomersPageProps> = (): ReactElement => {
  ...
  const dataUsageData = useMemo(() => {
    if (!userInfo?.subscriptionUsage?.usage || !userInfo?.subscriptionUsage?.usage[0]) {
      return {};
    }
          ...
          <div className="flex flex-wrap">
            ...
            {isUserInfoError ? (
              ...
              <>
                      ...
                      <div className="flex justify-between mb-6">
                        <div className="flex items-center">
                          <div className="flex flex-col">
                            <span className="font-bold text-lg text-black ml-2">Connection Status</span>
                            {(userInfo?.connectionStatus?.status).toUpperCase() === 'ACTIVE' ? (
                              ...
                              <>
                                <div className="rounded-lg border border-red-600 bg-white m-3 p-2">
                                  <div className="flex m-auto">
                                    <p className="text-xl font-bold text-red-600 mr-1">Inactive</p>
                                    <FcCancel size={25} />
                                  </div>
                                </div>
                  ...
                  <div className="w-full md:w-1/2 lg:w-1/3 pr-4 pb-4">
                    <div className="shadow-lg rounded-lg p-4 bg-white w-full">
                      <div className="flex justify-between mb-6">
                        <div className="flex items-center">
                          <div className="flex flex-col">
                            <span className="font-bold text-lg text-black ml-2">Customer Information</span>
                            {userInfo?.personalDetails?.phoneNumbers?.length > 0 && (
                    ...
                    <div className="shadow-lg rounded-lg p-4 bg-white relative overflow-hidden">
                      ...
                      <div className="flex justify-between mt-4">
                        ...
                        <p className="text-primary text-sm font-medium">
                          {currency.format(userInfo?.billingData?.currentBillingCycle?.amount)}
                        </p>
                      </div>
                ...
                <div className="w-full md:w-1/2 lg:w-1/3 pr-4 pb-4">
                  <div className="shadow-lg rounded-lg p-4 bg-white relative overflow-hidden">
                    <p className="text-gray-800 text-lg font-bold mb-2">Data Usage</p>
                    ...
                    userInfo?.subscriptionUsage?.usage[0].allocatedDataUsage ? (
                      ...
                      <span className="font-medium">No data packages are activated</span>
                    )}
                  </div>
                </div>
                ...
                <div className="w-full md:w-1/2 lg:w-1/3 pr-4 pb-4">
                  <div className="shadow-lg rounded-lg p-4 bg-white relative overflow-hidden">
                    <p className="text-gray-800 text-lg font-bold mb-2">Call Usage</p>
                    {userInfo?.subscriptionUsage?.usage &&
                    userInfo?.subscriptionUsage?.usage[0] &&
                    userInfo?.subscriptionUsage?.usage[0].allocatedMinutesUsage ? (
                      <>
                      ...
                      <span className="font-medium">No voice packages are activated</span>
                    )}
                  </div>
                </div>
                      ...
                      <div className="p-4 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                        <div className="flex flex-wrap -mx-3">
                          <div className="flex items-center flex-none w-1/2 max-w-full px-3">
                            <span className="font-bold text-lg text-black ml-2">Billing History</span>
                          </div>
                        </div>
                      </div>
                            ...
                            billingHistory.map((billingRecord, index) => (
                              ...
                              >
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-xs leading-tight">{billingRecord?.status}</span>
                                </div>
                                <div
                                  className={`flex items-center text-sm leading-normal ${
                                    billingRecord?.status === 'PENDING' ? 'text-primary' : ''
                                  }`}
                                >
