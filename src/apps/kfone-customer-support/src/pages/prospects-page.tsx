...
import {DetailedHTMLProps, FC, FormEvent, HTMLAttributes, ReactElement, useMemo, useState} from 'react';
...
import {SiFacebook, SiTwitter, SiLinkedin, SiGithub, SiCrunchbase} from 'react-icons/si';
import {DashboardLayout} from '../layouts';
import {PreLoader} from '../components';
import {Prospect360Response} from '../models';
import {getCustomer360View} from '../api/prospect';

/**
 * The props for the Prospects Page component.
 */
export type ProspectsPageProps = DetailedHTMLProps<HTMLAttributes<HTMLDivElement>, HTMLDivElement>;

/**
 * Prospects page component.
 *
 * @returns The prospects page component.
 */
export const ProspectsPage: FC<ProspectsPageProps> = (): ReactElement => {
  ...
  const personalInfo = useMemo(() => {
    ...
    return {
      ...
      location: person.geo ? person.geo.country + ` (${person.geo.countryCode})` : person.location,
  ...
  const hsData = useMemo(() => {
    ...
    return {
      whitepaperDownloads: hubspotData.no_whitepaper_downloads ? hubspotData.no_whitepaper_downloads : 0,
      webinarRegistrations: hubspotData.no_webinar_registrations ? hubspotData.no_webinar_registrations : 0,
  ...
  const getSentimentIcon = (score: number): ReactElement => {
    if (score >= 75) {
      return <MdSentimentVerySatisfied className="text-green-400" size={64} />;
    } else if (score >= 50) {
      return <MdSentimentSatisfied className="text-blue-400" size={64} />;
    } else if (score >= 25) {
      return <MdSentimentNeutral className="text-amber-400" size={64} />;
    } else return <MdSentimentDissatisfied className="text-red-400" size={64} />;
  };
    ...
    <>
      {state.isAuthenticated && !state.isLoading ? (
        <DashboardLayout>
          ...
          {isUserInfoLoading ? (
            <div
              className={'flex flex-col justify-center items-center top-1/2 right-[calc(50%-120px)] absolute space-y-4'}
            >
              <div role="status">
                <svg
                ...
                <div className="stat space-y-2">
                  {userInfo?.sentimentScore ? (
                    <div className="flex items-center space-x-4">
                      <div>{getSentimentIcon(userInfo?.sentimentScore)}</div>
                      <div>
                        <div
                          className={`stat-value text-6xl ${
                            userInfo?.sentimentScore >= 75
                              ? 'text-green-500'
                              : userInfo?.sentimentScore >= 50
                              ? 'text-blue-500'
                              : userInfo?.sentimentScore >= 25
                              ? 'text-amber-500'
                              : 'text-red-500'
                          }`}
                        >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {hsData?.stage ? (
                      <div
                        className={
                          'flex justify-between items-center py-2 px-4 rounded-full text-slate-500 bg-orange-50 text-xs'
                        }
                      >
                        <FaHubspot className="text-orange-500 mr-2" size={24} />
                        Hubspot Stage:&nbsp;
                        <span className="text-sm text-orange-800">{hsData?.stage?.toUpperCase()}</span>
                      </div>
                    ) : null}
                    {sfData?.sfStatus ? (
                      <div
                        className={
                          'flex justify-between items-center py-2 px-4 rounded-full text-slate-500 bg-cyan-50 text-xs'
                        }
                      >
                        <FaSalesforce className="text-cyan-500 mr-2" size={24} />
                        Salesforce Status:&nbsp;
                        <span className="text-sm text-cyan-800">{sfData?.sfStatus?.toUpperCase()}</span>
                      </div>
                    ) : null}
                    {sfData?.leadSource ? (
                      <div
                        className={
                          'flex justify-between items-center py-2 px-4 rounded-full text-slate-500 bg-pink-50 text-xs'
                        }
                      >
                        <VscSourceControl className="text-pink-500 mr-2" size={24} />
                        Lead source:&nbsp;
                        <span className="text-sm text-pink-800">{sfData?.leadSource}</span>
                      </div>
                    ) : null}
                    {snData?.status ? (
                      <div
                        className={
                          'flex justify-between items-center py-2 px-4 rounded-full text-slate-500 bg-green-50 text-xs'
                        }
                      >
                        <VscSymbolProperty className="text-green-500 mr-2" size={24} />
                        ServiceNow status:&nbsp;
                        <span className="text-sm text-green-800">{snData?.status}</span>
                      </div>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="card w-full shadow-xl bg-white text-secondary">
                  <div className="card-body">
                    ...
                    {personalInfo ? (
                          ...
                          <tbody>
                            {personalInfo.avatar ? (
                              <tr>
                                <th></th>
                                <td>
                                  <img className="w-20 h-20 rounded-full" src={personalInfo.avatar} alt=""></img>
                                </td>
                              </tr>
                            ) : null}
                            <tr>
                              <th>Name</th>
                              <td>{personalInfo.name ? personalInfo.name : sfData?.fullName}</td>
                            </tr>
                            <tr>
                              <th>Email</th>
                              <td>{personalInfo.email ? personalInfo.email : sfData?.email}</td>
                            </tr>
                            <tr>
                              <th>Title</th>
                            ...
                            <tr>
                              <th>Company</th>
                              <td>{personalInfo.company ? personalInfo.company : sfData?.company}</td>
                            </tr>
                            <tr>
                              <th>Location</th>
                      ...
                      <div className="text-sm text-gray-600">Data not available</div>
                    )}
                  </div>
                </div>
                <div className="card w-full shadow-xl bg-white text-secondary">
                          ...
                          <tbody>
                            {companyInfo.logo ? (
                              <tr>
                                <th></th>
                                <td>
                                  <img className="h-16" src={companyInfo.logo} alt=""></img>
                                </td>
                              </tr>
                            ) : null}
                            <tr>
              ...
              <div className="stats shadow-xl w-full overflow-x-auto mb-10">
                <div className="stat">
                  <div className="stat-figure text-secondary">
                    <HiOutlineNewspaper size={48} className="text-primary" />
                  </div>
                  <div className="stat-title">Whitepaper Downloads</div>
                  <div className="stat-value">{hsData?.whitepaperDownloads}</div>
                  <div className="stat-desc">Last updated on {hsData?.modifiedAt}</div>
                </div>

                <div className="stat">
                  <div className="stat-figure text-secondary">
                    <HiOutlineViewGridAdd size={48} className="text-primary" />
                  </div>
                  <div className="stat-title">Webinar Registrations</div>
                  <div className="stat-value">{hsData?.webinarRegistrations}</div>
                  <div className="stat-desc">Last updated on {hsData?.modifiedAt}</div>
                </div>

                <div className="stat">
                  <div className="stat-figure text-secondary">
                    <HiOutlineTrendingUp size={48} className="text-primary" />
                  </div>
                  <div className="stat-title">Interested Products</div>
                  <ul className="stat-value text-lg uppercase">
                    {hsData?.interestedProducts?.map((e: string, i: number) => (
                      <li className="flex items-center" key={i}>
                        <HiOutlineStar size={12} className="mr-1 opacity-60" />
                        {e}
                      </li>
                    ))}
                  </ul>
                    ...
                    <div className="card-body">
                      ...
                      <div className="overflow-x-auto">
                        <table className="table table-compact w-full">
                          ...
                          <tbody>
                            {snData.cases?.map((el: any, index: number) => (
                              <tr key={index}>
                                <td>{el.number}</td>
                                <td>{el.opened_at}</td>
                                ...
                                <td>
                                  {el.active === 'true' ? (
                                    <span className="flex items-center">
                                      Open&nbsp;
                                      <MdCircle size={10} color="#fb923c" />
                                    </span>
                                  ) : (
                                    <span className="flex items-center">
                                      Closed&nbsp;
                                      <MdCircle size={10} color="#4ade80" />
                                    </span>
                                  )}
                                </td>
                              </tr>
